...
stepwise.model = lm(log(early_career_pay) ~ make_world_better_percent +
                      stem_percent + avg_fac_salary + pct_pell + median_hh_income,
                    data=train)
plot(stepwise.model)
stepwise.preds = predict(stepwise.model, newdata = test)
stepwise.rmspe = RMSE(stepwise.preds, log(test$early_career_pay))
```
...
X.test.lasso = model.matrix(log(early_career_pay) ~ . -name-state-mid_career_pay-state_code
                        -`Total Minority`-out_of_state_total-`Non-Resident Foreign`
                        -in_state_total-region-Black-in_state_tuition-rank-poverty_rate
                        -pred_degree_type-highest_degree, data=test)[,-1]
coef(fit.lasso, s=lambda.lasso.chosen)
...
library(pls)
set.seed(704)
fit.pcr = pcr(log(early_career_pay) ~ . -name-state-mid_career_pay-state_code
                        -`Total Minority`-out_of_state_total-`Non-Resident Foreign`
                        -in_state_total-region-Black-in_state_tuition-rank-poverty_rate
                        -pred_degree_type-highest_degree,
              data=train,
...
# PLS
set.seed(704)
fit.pls = plsr(log(early_career_pay) ~ . -name-state-mid_career_pay-state_code
                        -`Total Minority`-out_of_state_total-`Non-Resident Foreign`
                        -in_state_total-region-Black-in_state_tuition-rank-poverty_rate
                        -pred_degree_type-highest_degree,
              data=train,
